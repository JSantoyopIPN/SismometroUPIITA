# -*- coding: utf-8 -*-
"""
Created on Sat Aug 13 22:50:16 2022
...
class SignalSensors():
    def getValue(self):
        serialDataASCII = serData.readline() # Lectura de datos desde Arduino (ASCII)
        return serialDataASCII

class SignalCommunicate(PyQt5.QtCore.QObject):
    LehmanSignal = PyQt5.QtCore.pyqtSignal(int, float)
    LaCosteSignal = PyQt5.QtCore.pyqtSignal(int, float)
    signalUpdate = PyQt5.QtCore.pyqtSignal()
...
class Ui_MainWindow(QtWidgets.QMainWindow):
    def __init__(self, signalUpdateInterval = 15):        
        ...
        # self.valueLehmanSignal.setValue('Prueba')
        # self.valueLehmanFFT.setText('Prueba')
        # self.valueLaCosteSignal.setText('Prueba')
        # self.valueLaCosteFFT.setText('Prueba')
        ...
        #self.pruebaA = int(self.valueLehmanSignalX.setValue(str(self.xLehman[0])))
        
        # Signals that can be emitted 
        self.signalComm = SignalCommunicate()
        self.signalComm.LehmanSignal.connect(int(self.valueLehmanSignalX.setText(str(self.xLehman[0]))), float(self.valueLehmanSignalY.setText(str(self.yLehman[0]))))
        # self.signalComm.LaCosteSignal.connect(self.xLaCoste[0], self.yLaCoste[0])
        # self.signalComm.signalUpdate.connect(self.updateSignalPlot)
        
        # self.signalLehmanUpdate = Thread(target=self.read_position, args=(self.my_sensor, self.sensor_update_interval))
